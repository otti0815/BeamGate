defmodule ReverseProxy.Search.Manager do
  @moduledoc false
  use GenServer

  alias ReverseProxy.Search.Index
  ...
  def handle_call({:create_index, index, config}, _from, state) do
    if Map.has_key?(state.indexes, index) do
      ...
      case DynamicSupervisor.start_child(ReverseProxy.Search.IndexSupervisor, spec) do
        {:ok, pid} ->
  ...
  def handle_call({:delete_index, index}, _from, state) do
    case Map.fetch(state.indexes, index) do
      {:ok, %{pid: pid}} ->
        _ = DynamicSupervisor.terminate_child(ReverseProxy.Search.IndexSupervisor, pid)
