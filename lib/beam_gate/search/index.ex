defmodule BeamGate.Search.Index do
  @moduledoc false
  use GenServer

  alias BeamGate.Search.QueryCoordinator
  alias BeamGate.Search.Shard
  ...
  def terminate(_reason, state) do
    for pid <- Map.values(state.primaries) do
      _ = DynamicSupervisor.terminate_child(BeamGate.Search.ShardSupervisor, pid)
    end
  ...
  defp start_primary_shards(index, shard_ids, mappings, analysis) do
    Enum.reduce_while(shard_ids, {:ok, %{}}, fn shard_id, {:ok, acc} ->
      ...
      case DynamicSupervisor.start_child(BeamGate.Search.ShardSupervisor, spec) do
